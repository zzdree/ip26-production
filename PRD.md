...
| **FR-04** | **Interactive Flowchart, 2-Column Subsystems & Transparent Mermaid Engine** | Dual-engine visual diagrams: (1) High-level interactive 4-stage hardware pipeline board, and (2) 8 embedded Mermaid.js architecture flowcharts with transparent node containers, horizontal `flowchart LR` layouts, and auto-adapting lines/text (pure light text in dark mode, deep charcoal in light mode). Deep-dive sub-systems are laid out in an ergonomic **2-column responsive grid (6 balanced cards)** with dynamic 2.5px glowing outlines on hover/click and instant re-render on theme toggle without page reload. | P0 |
| **FR-05** | **Realtime Multi-Device Cloud Sync (Supabase-First)** | 100% cloud-driven multi-crew bi-directional checklist synchronization using Supabase PostgreSQL + Realtime WebSockets CDC. Eliminates local storage desync. Tracks independent 'Loading In' (pemasangan) and 'Packing Out' (pengemasan) statuses per item via tactile checkboxes (`✓`), with dynamic total item calculation, crew identity badges, and real-time presence indicators (`online crew count`). | P0 |
...
2. **Accessibility (a11y):** WCAG 2.1 AA compliant contrast ratios in both dark and light modes; keyboard accessible navigation (`Tab`, `Escape` modals, `Skip-to-content` link); clean hairline default borders with high-visibility focused outlines on active elements.
...
8. **Master Inventory Directory:** Live search, 14 vendor filter pills, dual-action realtime checklist (`[v] Pasang` & `[v] Kemas`), dynamic progress gauges, and clipboard summary exporter.
