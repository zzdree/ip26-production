...
| **FR-04** | **Interactive Flowchart & Transparent Mermaid Engine** | Dual-engine visual diagrams: (1) High-level interactive 4-stage hardware pipeline board, and (2) 7 embedded Mermaid.js architecture flowcharts with transparent node containers and auto-adapting lines/text (pure light text in dark mode, deep charcoal in light mode) that dynamically re-render on theme toggle without page reload. | P0 |
| **FR-05** | **Realtime Multi-Device Cloud Sync (Supabase-First)** | 100% cloud-driven multi-crew bi-directional checklist synchronization using Supabase PostgreSQL + Realtime WebSockets CDC. Eliminates local storage desync. Tracks independent 'Loading In' (pemasangan) and 'Packing Out' (pengemasan) statuses per item via tactile checkboxes (`✓`), with crew identity badges and real-time presence indicators (`online crew count`). | P0 |
