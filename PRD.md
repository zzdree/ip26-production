...
| **FR-06** | **Live Inventory Search, Multi-Filter & Fixed-Width Tables** | Instant client-side search and category filtering across all 14 equipment providers (OWL, ABON, Andreas, GIA, GKJ, UKK, etc.) with real-time status badges (`✅ Ready`, `⚠️ Belum Ada / Sebagian`, `☑️ Reserve`). Tables across all 14 vendor sections feature strict fixed column alignment (`table-layout: fixed`) and single-line description ellipsis with hover tooltips to ensure complete vertical alignment across all categories. | P0 |
| **FR-07** | **Camera Rig Layouts (2-Col Broadcast & 3-Col Documentation)** | Dedicated visual cards: Broadcast cameras arranged in a clean **2-column grid** (CAM 1 & 2 on top row, CAM 3 & 4 on bottom row) and Documentation cameras in a **3-column grid** (CAM PHO, CAM VID, CAM HP). | P1 |
...
5. **6 Sub-System Deep-Dives (2-Column Grid):** (1) Wireless Cameras (CAM 1 & 2), (2) Wired Cameras (CAM 3 & 4), (3) Video & LED Mapping, (4) Audio Routing & Streaming, (5) Stage Time Keeper, and (6) Electrical Power Distribution (each with its dedicated compact `flowchart LR` Mermaid diagram & technical explanation).
