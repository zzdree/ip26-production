...
| **FR-04** | **Interactive Flowchart, 2-Column Subsystems & Transparent Mermaid Engine** | Dual-engine visual diagrams: (1) High-level interactive 4-stage hardware pipeline board, and (2) 7 embedded Mermaid.js architecture flowcharts with transparent node containers, horizontal `flowchart LR` layouts, and auto-adapting lines/text (pure light text in dark mode, deep charcoal in light mode). Deep-dive sub-systems are laid out in an ergonomic **2-column responsive grid** with dynamic 2.5px glowing outlines on hover/click and instant re-render on theme toggle without page reload. | P0 |
...
5. **5 Sub-System Deep-Dives (2-Column Grid):** Camera Acquisition, Video & LED Mapping, Audio Routing & Streaming, Stage Time Keeper, and Electrical Power Distribution (each with its dedicated compact `flowchart LR` Mermaid diagram & technical explanation).
