...
Upgraded with:
1. Real-Time Cloud & Local Sync for Master Inventory (MQTT over WebSockets + Offline QR Code Sync + LocalStorage)
2. Interactive Broadcast Wiring & Signal Flow Simulator (Live Switcher Cut Practice, Animated Pulsing Beams, Layer Filters, Fault Injection & Quick SOP)
"""
...
<head>
  ...
  <!-- External CDNs for Realtime Cloud Sync & QR Tools (With Local Fallback) -->
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .brand-subtitle { font-size: 0.75rem; color: var(--text-muted); font-family: var(--font-mono); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }

    .header-nav { display: flex; align-items: center; gap: 0.35rem; }
    .nav-link {
      padding: 0.45rem 0.85rem; font-size: 0.85rem; font-weight: 600; color: var(--text-secondary);
    ...
    .status-pill {
      ...
      border-radius: 9999px; font-size: 0.75rem; font-weight: 700; font-family: var(--font-mono);
    ...
    .btn {
      ...
      padding: 0.55rem 1rem; border-radius: var(--radius-sm); font-size: 0.85rem; font-weight: 600;
    ...
    .btn-sm { padding: 0.35rem 0.65rem; font-size: 0.75rem; border-radius: var(--radius-xs); }
    .btn-icon { width: 36px; height: 36px; padding: 0; border-radius: var(--radius-sm); }

    /* Layout Containers */
    .app-container { max-width: 1440px; margin: 0 auto; padding: 1.5rem; display: flex; flex-direction: column; gap: 2rem; }
    ...
    .section-heading { font-size: 1.5rem; font-weight: 700; color: var(--text-pure); }
    ...
    .hero-banner {
      ...
      border: 1px solid var(--border-medium); border-radius: var(--radius-lg); padding: 1.75rem;
    ...
    .hero-title { font-size: 2.1rem; font-weight: 800; letter-spacing: -0.04em; color: var(--text-pure); }
    .hero-meta { display: flex; flex-wrap: wrap; gap: 1.25rem; margin-top: 0.25rem; }
    .meta-item { display: flex; align-items: center; gap: 0.4rem; font-size: 0.85rem; color: var(--text-secondary); }
    .meta-icon { color: var(--text-muted); font-size: 1rem; }
    ...
    .telemetry-label { font-size: 0.7rem; font-weight: 700; text-transform: uppercase; color: var(--text-muted); font-family: var(--font-mono); }
    .telemetry-val { font-size: 1.35rem; font-weight: 800; color: var(--text-pure); font-family: var(--font-display); }
    ...
    .toast {
      ...
      padding: 0.75rem 1.25rem; border-radius: var(--radius-sm); font-size: 0.82rem; color: var(--text-pure);
    ...
    /* Camera Section */
    .cameras-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 1.25rem; }
    .cam-card {
      background: var(--bg-surface); border: 1px solid var(--border-subtle); border-radius: var(--radius-md);
      padding: 1.25rem; display: flex; flex-direction: column; gap: 0.85rem; position: relative;
    }
    ...
    .cam-op-name { font-size: 1.15rem; font-weight: 700; color: var(--text-pure); }
    ...
    .cam-specs-table td:first-child { color: var(--text-muted); font-family: var(--font-mono); width: 38%; }
    ...
    .cheat-sheet-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 1rem; margin-top: 0.75rem; }
    ...
    .cheat-item-desc { font-size: 0.8rem; color: var(--text-primary); }

    /* WIRING & SIGNAL ROUTING SIMULATOR */
    .sim-control-bar {
      display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 1rem;
      padding: 1rem 1.25rem; background: var(--bg-surface-raised); border: 1px solid var(--border-medium);
      border-radius: var(--radius-md) var(--radius-md) 0 0; border-bottom: none;
    }
    .sim-tabs { display: flex; gap: 0.35rem; flex-wrap: wrap; }
    .sim-tab-btn {
      padding: 0.4rem 0.75rem; font-size: 0.78rem; font-weight: 600; border-radius: var(--radius-xs);
    ...
    .sim-cut-deck { display: flex; align-items: center; gap: 0.4rem; flex-wrap: wrap; }
    .sim-cut-btn {
      padding: 0.35rem 0.65rem; font-size: 0.75rem; font-weight: 700; font-family: var(--font-mono);
      border-radius: var(--radius-xs); background: var(--bg-surface); color: var(--text-secondary);
      border: 1px solid var(--border-subtle); cursor: pointer; transition: var(--transition-smooth);
    }
    ...
    .schematic-canvas-wrap {
      ...
      overflow-x: auto; position: relative; padding: 1rem; box-shadow: inset 0 0 40px rgba(0, 0, 0, 0.8);
    }
    .schematic-svg { width: 100%; min-width: 1080px; height: 580px; display: block; }
    ...
    /* Node Inspector Panel below schematic */
    .sim-inspector-grid {
    ...
    .switcher-deck-card {
      ...
      border: 1px solid var(--border-strong); border-radius: var(--radius-md); padding: 1.5rem;
    ...
    .bus-label { font-size: 0.75rem; font-weight: 800; font-family: var(--font-mono); color: var(--text-muted); width: 45px; }
    .bus-btn {
      width: 44px; height: 44px; border-radius: var(--radius-xs); background: var(--bg-surface-raised);
      border: 2px solid var(--border-medium); color: var(--text-pure); font-weight: 800; font-family: var(--font-mono);
      cursor: pointer; transition: var(--transition-smooth); font-size: 0.9rem;
    }
    ...
    .search-input {
      ...
      padding: 0.5rem 0.85rem; color: var(--text-pure); font-size: 0.85rem; font-family: inherit; min-width: 240px;
    }
    .search-input:focus { outline: none; border-color: var(--accent-cyan); }
    .filter-pills { display: flex; gap: 0.35rem; flex-wrap: wrap; }
    .filter-pill {
      padding: 0.35rem 0.65rem; border-radius: var(--radius-xs); font-size: 0.75rem; font-weight: 600;
      background: var(--bg-surface); color: var(--text-secondary); border: 1px solid var(--border-subtle);
      cursor: pointer; transition: var(--transition-smooth);
    }
    ...
    .inv-card {
      ...
      transition: var(--transition-smooth); cursor: pointer; user-select: none;
    }
    ...
    .inv-name { font-size: 0.88rem; font-weight: 700; color: var(--text-pure); }
    .inv-qty { font-size: 0.75rem; font-weight: 800; font-family: var(--font-mono); color: var(--accent-cyan); background: rgba(6, 182, 212, 0.12); padding: 0.15rem 0.4rem; border-radius: 4px; }
    ...
    .timer-display { font-size: 4.5rem; font-weight: 800; font-family: var(--font-mono); letter-spacing: -0.04em; color: var(--text-pure); line-height: 1; }
    ...
    .timer-presets { display: flex; gap: 0.5rem; flex-wrap: wrap; justify-content: center; }
    .timer-btn-preset { padding: 0.35rem 0.75rem; font-size: 0.75rem; font-weight: 600; border-radius: var(--radius-xs); background: var(--bg-surface-raised); color: var(--text-secondary); border: 1px solid var(--border-subtle); cursor: pointer; }
    .timer-btn-preset:hover { color: var(--text-pure); background: var(--bg-surface-elevated); }

    /* Incident Logger */
    .incident-table-wrap { overflow-x: auto; margin-top: 0.75rem; }
    .incident-table { width: 100%; border-collapse: collapse; font-size: 0.8rem; }
    ...
    .modal-dialog {
      ...
      max-width: 800px; width: 100%; max-height: 90vh; overflow-y: auto; padding: 1.75rem; display: flex; flex-direction: column; gap: 1.25rem;
      box-shadow: var(--shadow-deep);
    }
    ...
    .floating-dock {
      ...
      background: var(--bg-glass); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px);
      border: 1px solid var(--border-medium); border-radius: 9999px; padding: 0.35rem 0.5rem;
      display: flex; align-items: center; gap: 0.25rem; box-shadow: var(--shadow-deep);
    }
    .dock-btn {
      padding: 0.45rem 0.75rem; border-radius: 9999px; font-size: 0.75rem; font-weight: 700;
    ...
    @media print {
      ...
      .command-header, .floating-dock, .btn, .no-print { display: none !important; }
...
<body>
  ...
  <header class="command-header">
    <a href="#" class="brand-cluster">
      ...
      <div class="brand-text-block">
        ...
        <span class="brand-subtitle">IBADAH PERDANA UKK UNNES 2026 • AUDITORIUM</span>
      </div>
    </a>
    ...
    <div class="header-actions">
      ...
      <button class="btn btn-secondary btn-sm" id="btn-open-briefing">📊 Deck</button>
      <button class="btn btn-primary btn-sm" id="btn-open-tally-modal">🚨 Tally</button>
    </div>
  </header>
  ...
  <main class="app-container">
      ...
      <div class="hero-banner">
        ...
        <div class="telemetry-grid">
          <div class="telemetry-card">
            ...
            <span class="telemetry-sub">2 Wireless RF + 2 SDI/HDMI</span>
          </div>
          <div class="telemetry-card">
          ...
          <div class="telemetry-card">
            ...
            <span class="telemetry-sub">Dante + Submixer CT80S</span>
          </div>
          <div class="telemetry-card">
    ...
    <section id="cameras">
      <div class="section-head">
        <div class="section-title-wrap">
          ...
          <h2 class="section-heading">Broadcast & Documentation Rigs</h2>
        </div>
      </div>

      <div class="cameras-grid" id="cameras-cards-grid">
        <!-- Rendered dynamically by JavaScript -->
      </div>

      <!-- Sony Alpha Cheat Sheet -->
      <div class="cheat-sheet-box">
        <h3 style="font-size:1rem; font-weight:700; color:var(--text-pure);">📷 Sony Alpha / ZV-E10 Live Broadcast Best Practices</h3>
    ...
    <section id="crew">
      <div class="section-head">
        <div class="section-title-wrap">
          ...
          <h2 class="section-heading">Broadcast & Media Personnel</h2>
        </div>
        <div class="section-actions">
          <div class="filter-pills" id="crew-filter-pills">
            ...
            <button class="filter-pill" data-crew-filter="docs">Dokumentasi</button>
          </div>
        </div>
      </div>
      ...
      <div class="card" style="padding:0; overflow:hidden;">
        ...
        <div class="sim-control-bar">
          ...
          <div class="sim-cut-deck">
            <span style="font-size:0.75rem; font-weight:700; color:var(--text-muted); font-family:var(--font-mono); margin-right:0.25rem;">SIMULASI CUT:</span>
        ...
        <div class="schematic-canvas-wrap">
          <svg class="schematic-svg" id="topo-svg" viewBox="0 0 1100 560">
            ...
            <!-- SIGNAL WIRES / PATHS -->
            <!-- Video & Wireless Inputs to Switcher -->
      ...
      <div class="switcher-deck-card">
        ...
        <div class="switcher-controls-row">
          ...
          <div style="display:flex; gap:0.5rem;">
            <button class="btn btn-primary" id="btn-sw-cut">⚡ CUT (Space)</button>
    ...
    <section id="inventory">
      <div class="section-head">
        <div class="section-title-wrap">
          ...
          <h2 class="section-heading">Master Equipment Inventory Log</h2>
        </div>
        <div class="section-actions">
      ...
      <div class="inventory-toolbar">
        ...
        <div class="filter-pills" id="inventory-filter-pills">
      ...
      <div style="display:grid; grid-template-columns: 1fr 1.5fr; gap:1.25rem;">
        ...
        <div class="card">
          ...
          <div style="display:flex; gap:0.5rem; margin-bottom:0.5rem;">
            <select id="incident-sev" class="search-input" style="min-width:110px; font-size:0.75rem; padding:0.35rem 0.5rem;">
              <option value="info">INFO</option>
              <option value="warn">WARNING</option>
              <option value="critical">CRITICAL</option>
            </select>
            <input type="text" id="incident-input" class="search-input" placeholder="Tulis kejadian / troubleshooting..." style="flex:1; font-size:0.78rem;" />
    ...
    <section id="sop">
      ...
      <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap:1.25rem;">
        <div class="card">
          <h3 style="font-size:1rem; font-weight:700; color:#ef4444; margin-bottom:0.5rem;">🚨 SOP Darurat: Blackout / HDMI Drop</h3>
        ...
        <div class="card">
          <h3 style="font-size:1rem; font-weight:700; color:#f59e0b; margin-bottom:0.5rem;">⚡ SOP Distribusi Daya Listrik</h3>
          <p style="font-size:0.8rem; color:var(--text-secondary); line-height:1.5;">
        ...
        <div class="card">
          <h3 style="font-size:1rem; font-weight:700; color:#10b981; margin-bottom:0.5rem;">🎛️ SOP Audio Ground Loop & Hum</h3>
          <p style="font-size:0.8rem; color:var(--text-secondary); line-height:1.5;">
  ...
  <div class="floating-dock">
    <a href="#overview" class="dock-btn active">Overview</a>
    <a href="#cameras" class="dock-btn">Cams</a>
    <a href="#crew" class="dock-btn">Crew</a>
    <a href="#routing" class="dock-btn">Routing</a>
    <a href="#switcher" class="dock-btn">Switch</a>
    <a href="#inventory" class="dock-btn">Inventory</a>
    <a href="#rundown" class="dock-btn">Rundown</a>
  </div>
  ...
  <script>
    ...
    const BROADCAST_CAMERAS = [
      ...
      { id: 3, name: 'CAM 3', operator: 'Dewi', model: 'Sony A6000', lens: 'Sony 18-105mm F4 G', trans: 'Direct Wired HDMI', cable: 'Micro HDMI Converter + HDMI 10M', mount: 'Tripod Camera Big', storage: 'SD 32GB', position: 'Left Wing Wide' },
      { id: 4, name: 'CAM 4', operator: 'Nathania', model: 'Sony A6000', lens: 'Sony Kit 16-50mm', trans: 'Direct Wired HDMI', cable: 'Micro HDMI Converter + HDMI 10M', mount: 'Tripod Camera Big', storage: 'SD 32GB', position: 'Right Wing Wide' }
    ];
    ...
    class MasterCommandApp {
      ...
      // 1. CAMERAS
      renderCameras() {
        const grid = document.getElementById('cameras-cards-grid');
        if (!grid) return;
        
        const bCards = BROADCAST_CAMERAS.map(c => `
          <div class="cam-card">
        ...
        const dCards = DOCS_CAMERAS.map(d => `
          <div class="cam-card" style="border-color:rgba(168, 85, 247, 0.3);">
            <div class="cam-header">
      ...
      renderCrew(filter = 'all') {
        ...
        if (!grid) return;
        const filtered = filter === 'all' ? CREW_ROSTER : CREW_ROSTER.filter(c => c.div === filter);
        grid.innerHTML = filtered.map(c => `
          <div class="card" style="padding:1rem; display:flex; align-items:center; justify-content:space-between; gap:0.5rem;">
            <div>
              <div style="font-size:1.05rem; font-weight:700; color:var(--text-pure);">${c.name}</div>
              <div style="font-size:0.8rem; color:var(--accent-cyan); font-weight:600;">${c.role}</div>
              <div style="font-size:0.72rem; color:var(--text-muted); font-family:var(--font-mono);">${c.note}</div>
            </div>
            <span class="badge-sev info">${c.div.toUpperCase()}</span>
          </div>
        `).join('');
      ...
      renderInventory(providerFilter = 'all', searchQuery = '') {
        const grid = document.getElementById('inventory-items-grid');
        if (!grid) return;

        let items = MASTER_INVENTORY;
        if (providerFilter !== 'all') items = items.filter(i => i.provider === providerFilter);
        if (searchQuery.trim()) {
        ...
        grid.innerHTML = items.map(item => {
          ...
          return `
            <div class="inv-card ${isChecked ? 'checked' : ''}" data-item-id="${item.id}">
              ...
              <div style="font-size:0.72rem; color:var(--text-secondary);">${item.note || ''}</div>
            </div>
          `;
        }).join('');
        ...
        document.querySelectorAll('#inventory-filter-pills .filter-pill').forEach(btn => {
          btn.addEventListener('click', (e) => {
            ...
            this.renderInventory(e.target.dataset.prov, searchVal);
          });
        });
        ...
        if (sInput) {
          sInput.addEventListener('input', (e) => {
            const activePill = document.querySelector('#inventory-filter-pills .filter-pill.active');
            const prov = activePill ? activePill.dataset.prov : 'all';
            this.renderInventory(prov, e.target.value);
          });
        }
      }
      ...
      toggleInventoryItem(id, fromSync = false, author = null) {
        ...
        // If toggled locally, publish via MQTT
        ...
        const prov = activePill ? activePill.dataset.prov : 'all';
        const searchVal = document.getElementById('inventory-search')?.value || '';
        this.renderInventory(prov, searchVal);
        this.updateInventoryProgress();
        ...
        try {
          ...
          this.mqttClient.onMessageArrived = (msg) => {
            try {
              ...
              if (payload.author === this.syncUser) return; // ignore own echo
      ...
      initToneGenerator() {
        ...
        document.getElementById('btn-toggle-tone')?.addEventListener('click', (e) => {
          if (!isTonePlaying) {
            ...
            osc.type = 'sine';
            osc.frequency.setValueAtTime(1000, audioCtx.currentTime); // 1kHz
            gain.gain.setValueAtTime(0.125, audioCtx.currentTime); // -18dBFS
            osc.connect(gain);
      ...
      initBriefingDeck() {
        ...
        const openBtn = document.getElementById('btn-open-briefing');
...
"""
    with open('index.html', 'w', encoding='utf-8') as f:
        f.write(html_code)
    print("index.html successfully updated with Wiring Simulator & Real-time Inventory Cloud Sync!")
